...
from evalharness.db.models import GenerationRow
...
def test_stored_generation_preserves_tool_calls_list() -> None:
    ...
    generation = _stored_generation(row, "case-ext-1").as_generation()  # type: ignore[arg-type]

    assert generation.tool_calls == tool_calls
    assert isinstance(generation.tool_calls, list)


def test_stored_generation_non_list_tool_calls_becomes_empty() -> None:
    row = SimpleNamespace(
        id=43,
        run_id=uuid.uuid4(),
        case_id=8,
    ...
    generation = _stored_generation(row, "case-ext-2").as_generation()  # type: ignore[arg-type]
